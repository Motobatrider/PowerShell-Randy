$objOU=[ADSI]"LDAP://OU=China,OU=Users,OU=Root2,DC=vstage,DC=co"
...
$Path=$user.Path
$account=[ADSI]$Path
$sName=$account.sAMAccountName.Value
$date1 = Get-ADUser -Identity $sName -Properties *| Select LastLogonDate
$date=$date1.lastLogonDate
...
$sheet.cells.item($intRow,6) = $date
